...
amenities_to_drop = list(filter(lambda column: 'translation missing' in column, df_amenities.columns))

df_amenities = df_amenities[['ID'] + [ col for col in df_amenities.columns if col not in ['ID', *amenities_to_drop]]] # esto es solo para que ID sea la primera columna
# esto es solo para que ID sea la primera columna

print(df_amenities.columns)
# Conectar con database de PostgreSQL
### ‘postgresql://username:password@databasehost:port/databasename’
db_url = 'postgresql://aanckzrm:jXHvLLGTx4AaFlKkQSF6KaJAtK-Zh3sv@trumpet.db.elephantsql.com/aanckzrm'

psql_engine = create_engine(db_url)
df.to_sql('Airbnbs', psql_engine)
df_amenities.to_sql('Amenities', psql_engine)